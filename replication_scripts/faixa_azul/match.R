...
padronizar <- function(coluna, titulo, preposicao, tipo, numero) {
  coluna |>
    ...
    str_replace_all(setNames(
      rep(" ", length(preposicao$preposicao)),
      limpar(preposicao$preposicao)
    )) |> #remover preposicao
...
tokenizar <- function(df, id) {
  ...
  df |>
    ...
    mutate(
      logradouro_limpo = logradouro |>
        padronizar(titulo, preposicao, tipo, numero) |>
        str_replace_all(
          string = _,
          setNames(rep(" ", length(titulo$titulo_ext)), titulo$titulo_ext)
        ) |>
        str_replace_all(
          string = _,
          setNames(rep(" ", length(tipo$tipo)), tipo$tipo)
        ) |>
        limpar()
    ) |>
    fuzzyjoin::regex_left_join(
      tipo |> select(tipo),
      by = join_by(logradouro == tipo)
    ) |>
    fuzzyjoin::regex_left_join(
      titulo |> select(titulo = titulo_ext),
      by = join_by(logradouro == titulo)
    ) |>
    ...
    summarize(
      logradouro_limpo = nth(logradouro_limpo, 1),
      tipo = tipo |>
        str_split(" ") |>
        unlist() |>
        unique() |>
...
match_dados <- function(sinistros, sinistros_token, trechos, trechos_token) {
  ...
  match_nome <- join |>
    ...
    mutate(
      similaridade = stringdist::stringsim(
        logradouro_limpo.x,
        logradouro_limpo.y
      ),
    ...
    mutate(
      distancia_nome = stringdist::stringdist(
        logradouro_limpo.x,
        logradouro_limpo.y
      )
    )
  ...
  match_grafico <- match_nome |>
    ...
    filter(
      row_number() ==
        st_nearest_feature(nth(geometria_ponto, 1), geometria_trecho)
    ) |>
    mutate(
  ...
  match <- match_grafico |>
    ...
    group_by(id_sinistro) |>
    filter(
      match_tipo + match_titulo == max(match_tipo + match_titulo), #Selecionar o que tem maior match de título e tipo
      match_titulo == max(match_titulo), #Se um tiver match e título, e o outro match no tipo, manter apenas match no título
      row_number() == 1
    ) #Garantir que sobra apenas uma linha no matter what
