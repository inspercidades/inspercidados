plot_obitos_ano <- function(sinistros, vitimas) {
  gg <- vitimas |>
    ...
    mutate(
      tipo_veiculo_vitima = str_to_upper(tipo_veiculo_vitima),
      veiculo = fct_collapse(
        tipo_veiculo_vitima,
        other_level = "outros",
    ...
    mutate(
      y = case_when(
        veiculo == "motocicleta" ~ obitos,
        veiculo == "outros" ~ sum(obitos)
      ),
      label = case_when(
        veiculo == "motocicleta" ~ scales::percent(obitos / sum(obitos)),
...
plot_datas_FA <- function(logradouros, logradouros_id, match, sinistros) {
  ...
  gg <- logradouros |>
    ...
    scale_fill_manual(
      NULL,
      values = c(
        "Antes da implementação" = "grey90",
        "Depois da implementação" = "#333F48FF"
      )
    ) +
    labs(y = NULL, x = NULL) +
    theme_minimal()
  ...
  ggg <- gg +
    new_scale_fill() +
    geom_point(
      aes(x = data, fill = motocicleta, shape = motocicleta),
      ...
      data = logradouros_id |>
        ...
        inner_join(
          match |> filter(golden_match) |> select(id_sinistro, id_osm)
        ) |>
        left_join(
          sinistros |>
    ...
    # geom_jitter() +
    scale_fill_manual(
      "Modo de transporte da vítima",
      values = c("TRUE" = "#BA0C2FFF", "FALSE" = "#C6AA76FF"),
      labels = c("TRUE" = "Motocicleta", "FALSE" = "Outros")
    ) +
    scale_shape_manual(
      "Modo de transporte da vítima",
...
plot_tamanho_FA <- function(logradouros_id, logradouros, faixa_azul, trechos) {
  gg <- logradouros_id |>
    ...
    scale_fill_manual(
      "",
      values = c("FALSE" = "#A6A6A6", "TRUE" = "#4472C4"),
...
plot_obitos_tempo <- function(sinistros, vitimas, match, agregados) {
  gg <- vitimas |>
    ...
    theme_minimal()
  ggsave(
    "output/plot - obitos anuais por modo (faixa azul).pdf",
    gg,
    width = 9,
    height = 5
  )
...
plot_datas_trechos <- function(faixa_azul, trechos, token_osm) {
  df <- faixa_azul |>
    ...
    summarize(trechos = sum(comprimento, na.rm = T) / 1000) |>
    ungroup() |>
    complete(
      data_implementacao = seq(
        min(data_implementacao),
        max(data_implementacao),
        by = "1 month"
      ),
      logradouro,
      fill = list(trechos = 0)
  ...
  gg <- df |>
    ...
    labs(
      x = NULL,
      y = "Extensão (km) dos trechos com faixa azul",
      title = NULL
    ) +
    theme_minimal() +
    # scale_fill_viridis_d(direction = -1) +
    scale_fill_manual(
      "Logradouros que receberam faixa azul (em ordem de implementação)\n",
  ...
  ggsave(
    "output/plot - extensao da faixa azul por mes.pdf",
    gg,
    width = 12,
    height = 10
  )
...
plot_qualidade_match <- function(sinistros, match) {
  tabela <- sinistros |>
    ...
    left_join(match) |>
    mutate(
      qualidade_match = case_when(
        similaridade == 1 &
          distancia_geografica < 50 &
          match_tipo &
          match_titulo &
          numero_zero == FALSE ~ "Perfeito",
        similaridade > .85 &
          distancia_geografica < 150 &
          (match_tipo | match_titulo) &
          numero_zero == FALSE ~ "Excelente",
  ...
  gg <- tabela |>
    ...
    geom_col(aes(x = n)) +
    geom_text(
      aes(x = n, label = percent |> round(3) |> scales::percent()),
      nudge_x = 7500
    ) +
    scale_x_continuous(
      labels = scales::number,
      expand = expansion(mult = c(0.05, 0.075))
    ) +
    theme_minimal() +
    theme(
...
plot_mapas <- function(sinistros, trechos, faixa_azul) {
  ...
  gg <- ggplot() +
    ...
    geom_sf(
      ...
      aes(geometry = st_simplify(geometry, dTolerance = 10)),
      colour = "grey50",
      lwd = .15,
      alpha = .8
    ) +
  ...
  df <- faixa_azul |>
    ...
    (\(df) {
      df |>
        ...
        mutate(
          grupo = rep(
            list(1:length(data_implementacao)),
            length(data_implementacao)
          )
        ) |>
        unnest(grupo) |>
        left_join(
...
plot_agregacao_trechos <- function(trechos, id_logradouros, agregados) {
  ...
  g <- bind_rows(list(
    ...
    id_logradouros |>
      ...
      summarize(comprimento = sum(comprimento), n = n()) |>
      select(comprimento) |>
  ...
  gg <- g |>
    ggplot() +
    geom_density(
      aes(fill = grupo, x = comprimento, colour = grupo),
      alpha = .6
    ) +
    geom_segment(
      data = g |>
    ...
    scale_x_continuous(
      "Comprimento da via (em metros) - escala log10",
      trans = "log10"
    ) +
  ...
  ggsave(
    "output/plot - extensao dos trechos por agregacao.pdf",
    gg,
    width = 9,
    height = 4.5
  )
...
plot_antes_depois <- function(sinistros, vitimas, agregados, match) {
  gg <- sinistros |>
    left_join(
      vitimas |>
        ...
        mutate(
          grupo = fct_collapse(
            str_to_upper(tipo_veiculo_vitima),
            motocicleta = "MOTOCICLETA",
            pedestre = "PEDESTRE",
            nao_disponivel = "NAO DISPONIVEL",
            other_level = "outros"
    ...
    scale_fill_manual(
      ...
      labels = c(
        obitos_motocicleta = "Motocicleta",
        obitos_outros = "Outros",
        obitos_pedestre = "Pedestre"
      ),
      values = c(
        obitos_outros = "grey40",
        obitos_pedestre = "grey80",
        obitos_motocicleta = "#4472C4"
      )
    ) +
  ...
  gg <- sinistros |>
    mutate(
      moto = tp_veiculo_motocicleta > 0 & !is.na(tp_veiculo_motocicleta),
    ...
    mutate(
      n = ifelse(
        make_date(year = 2025, month = 5) - months(dist) < data_implementacao,
        NA,
        n
      )
    ) |>
    ...
    scale_fill_manual(
      "Veículos envolvidos",
      labels = c("Não envolveu motocicleta", "Envolveu motocicleta"),
