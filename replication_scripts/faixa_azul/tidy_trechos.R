...
download_osm <- function() {
  if (!file.exists("dados_brutos/dado_osm.gpkg")) {
    ...
    return(st_read("dados_brutos/dado_osm.gpkg") |> rename(geometry = geom))
  }
}
...
tidy_trechos_bruto <- function(osm) {
  ...
  trechos <- osm |>
    ...
    mutate(
      value = ifelse(test = is.na(value.x), yes = value.y, no = value.x)
    ) |>
    pivot_wider(
      id_cols = c(id_osm, logradouro),
      names_from = name,
      values_from = value
    ) |>
...
agrupar_logradouros <- function(trechos, token_osm) {
  ...
  conexoes <- trechos |>
    ...
    left_join(
      token_osm |> select(id_osm, log.x = logradouro_limpo),
      by = join_by(id_osm.x == id_osm)
    ) |>
    left_join(
      token_osm |> select(id_osm, log.y = logradouro_limpo),
...
agregar_trechos <- function(trechos, faixa_azul, metros = 500) {
  ...
  conexoes <- trechos |>
    ...
    left_join(faixa_azul) |>
    mutate(
      data_implementacao = if_else(
        is.na(data_implementacao),
        "",
        as.character(data_implementacao)
      ),
    ...
    # Identificar todas as vias que se conectam, mas tem o mesmo nome
    filter(logradouro.x == logradouro.y, id_osm.x != id_osm.y)
  ...
  grafo.dfs <- function(subgrafo, grafico = FALSE) {
    ...
    subids <- enframe(
      ...
      left_join(
        enframe(resultado_dfs$dist, name = "id_osm", value = "dist"),
        by = join_by(id_osm)
      ) |>
      mutate(
        naumentou = replace_na(dist <= lag(dist), FALSE),
    ...
    dfs_edge_ids <- get.edge.ids(
      subgrafo,
      vp = as.vector(t(cbind(
        ordem[-length(ordem)],
        ordem[-1]
      )))
    )
  ...
  eulerian <- logradouros |>
    ...
    mutate(
      grafo = map(
        data,
        \(x) induced_subgraph(grafo, vids = pull(x, id_osm))
      ),
  ...
  trechos_agregado <- eulerian |>
    ...
    unnest(sub_ids) |>
    mutate(
      id_logradouro_sub = str_c(
        str_pad(id_logradouro, 5, pad = "0"),
        str_pad(subid, 3, pad = "0"),
        sep = "-"
      )
    ) |>
    ...
    mutate(
      comprimento_total = sum(comprimento) + 1,
      ...
      id_trecho_agregado = str_c(
        id_logradouro_sub,
        str_pad(grupo, 3, pad = "0"),
        sep = "-"
      )
    ) |>
  ...
  trechos_agregado_completo <- bind_rows(list(
    ...
    trechos |>
      ...
      mutate(
        id_trecho_agregado = str_c(
          str_c("A", str_pad(row_number(), 4, pad = "0")),
          "001",
          sep = "-"
        )
      ) |>
