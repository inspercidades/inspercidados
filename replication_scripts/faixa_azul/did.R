...
limpar_tabela_did <- function(did_tabela) {
  did_tabela |>
    mutate(
      across(
        c(filtro_sinistros, filtro_segmentos),
        ~ .x |> as.character() |> replace_na("TRUE")
      ),
      across(
        c(rodarPSM, filtrar_golden),
        ~ .x |> as.logical() |> replace_na(TRUE)
      ),
...
segmento_psm <- memoise::memoise(function(
  segmentos,
  sinistros,
  match,
  rodarPSM = TRUE,
  ...
  if (rodarPSM == TRUE) {
    ...
    medias_pre <- segmentos |>
      ...
      left_join(
        sinistros |>
          filter(year(data) >= 2019, year(data) <= 2021) |>
      ...
      group_by(ID) |>
      summarise(
        sinistros_fatais = sum(sinistro_fatal),
        sinistros_total = n()
      ) |>
      right_join(
        segmentos |>
    ...
    df <- segmentos |>
      ...
      mutate(
        faixa_azul = as.integer(!is.na(data_implementacao)),
        ...
        across(
          c(faixas, limite_velocidade, mao_unica),
          ~ ifelse(is.na(.x), "NA", .x)
        ),
    ...
    resultado <- df |>
      ...
      mutate(resultado_match = PSM$weights, propensity_score = PSM$distance) |>

      # Quando for especificado um propensity mínimo, remover os que não passam no critério
      mutate(
        resultado_match = if_else(
          propensity_score <= min_score_cut,
          0,
          resultado_match
        )
      )
...
sinistro_filtro <- memoise::memoise(function(
  sinistros,
  vitimas,
  filtro,
  apenas_moto = FALSE
  ...
  if (apenas_moto == TRUE) {
    sinistros <- sinistros |> filter(tp_veiculo_motocicleta > 0)
  }
...
agrega_tempo <- memoise::memoise(function(
  segmentos_filtrado,
  sinistros_filtrado,
  match,
  ...
  segmentos |>
    ...
    filter(
      if (filtrar_golden == TRUE) {
        eval(golden_match == TRUE)
      } else {
        TRUE
      },
      !is.na(id_sinistro),
    ...
    left_join(
      segmentos |>
        ...
        # Tornar datetimes mensais (desconsiderar o dia do mês)
        mutate(
          data_implementacao = make_date(
            year = year(data_implementacao),
...
fit_did <- function(
  ...
  if (!is.na(remover_formula)) {
    if (remover_formula == "tudo") {
      ...
      formula <- update(
        ...
        paste(
          " .~. -",
          paste(remover_formula |> str_split_1(","), collapse = "-")
        )
      )
    }
  }
  ...
  if (por_km) {
    df <- df |>
      mutate(across(c(y, intersec, amenidades), ~ .x * 1000 / comprimento))
  }
...
plot_did <- function(
  ...
  plot <- did |>
    aggte(
      type = "dynamic",
      min_e = -12 / intervalo_meses,
      max_e = 12 / intervalo_meses,
      na.rm = TRUE
    ) |>
    ggdid() +
    scale_y_continuous(expand = expansion(mult = expand_grid)) +
    scale_x_continuous(
      "Meses até data da implementação",
      breaks = c(0:9 - 4) * 3
    ) +
    scale_colour_manual(
      values = c("red", "blue"),
      labels = c("Pré faixa azul", "Pós faixa azul")
    ) +
    labs(title = title) +
    theme_minimal() +
  ...
  figura <- (wrap_table(tabela1, space = "fixed") /
    plot /
    wrap_table(tabela2, space = "fixed"))

  ggsave(
    paste0("output/did/", file, ".pdf"),
    figura,
    width = 7,
    height = 6,
    bg = "white",
...
summary_tabelinha_did <- function(did, nome, intervalo_meses = 1) {
  ...
  ci_low = ATT - se * 1.96
  ci_high = ATT + se * 1.96
  significance <- if (ci_low < 0 & ci_high > 0) {
    ""
  } else {
    "*"
  }
  ...
  tabelinha <- tibble(
    ...
    Significante = if (ci_low < 0 & ci_high > 0) {
      "Não"
    } else {
      "Sim"
    }
    ...
    (\(df) {
      bind_rows(
        ...
        df |>
          ...
          mutate(id = id - .1, across(c(everything(), -id), ~""))
      )
    })() |>
    arrange(id) |>
    select(-id) |>
