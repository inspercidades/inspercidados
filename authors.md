# Authors and Citation

## Authors

- **[Vinicius Oike](https://github.com/viniciusoike)**. Author,
  maintainer. [](https://orcid.org/0009-0005-8015-9189)

- **Insper Cidades**. Copyright holder, funder.

## Citation

Source:
[`DESCRIPTION`](https://github.com/inspercidades/inspercidados/blob/main/DESCRIPTION)

Oike V (2026). *inspercidados: Access to Datasets from Insper Cidades*.
R package version 0.4.0,
<https://github.com/inspercidades/inspercidados>.

    @Manual{,
      title = {inspercidados: Access to Datasets from Insper Cidades},
      author = {Vinicius Oike},
      year = {2026},
      note = {R package version 0.4.0},
      url = {https://github.com/inspercidades/inspercidados},
    }
