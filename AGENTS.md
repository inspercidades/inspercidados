...
files. When the sheet carries an `alias` column, it supplies the primary alias
for each catalog row, overriding `aliases.csv`; `aliases.csv` still provides
sub-aliases, because one deposit can hold several datasets and a single sheet
column cannot say that. `is_spatial` and `formats` are derived from the
...
Delimited text goes through `get_dataframe_by_name(original = TRUE)`. Binary
formats (rds, parquet, gpkg, geojson, xlsx, gzipped tables) download via
`dataverse::get_file_by_name()` and are parsed locally. File listing uses
`dataverse::dataset_files()`. Metadata fetching uses `dataverse::get_dataset()`.
...
`docs = TRUE` returns a named list with `data` and `docs`: the `docs` element
is a `documentacao*.xlsx` workbook when the deposit carries one, otherwise a
named list of Dataverse metadata (title, description, authors, DOI, URL, year).
...
- Network examples are gated with `@examplesIf live_examples()`: they run
  during `devtools::check()`, `devtools::run_examples()`, and pkgdown builds
  (`NOT_CRAN` / `IN_PKGDOWN`), and are skipped on CRAN, offline, or when the
  server is down
