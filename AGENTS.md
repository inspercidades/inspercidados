...
**inspercidados** is an R package that provides simple, reproducible
access to curated Brazilian urban research datasets hosted on Insper’s
Dataverse. It is aimed at researchers who want clean data in R with
minimal friction.

The package is a thin wrapper around the `dataverse` R package. Insper’s
Dataverse is the single source of truth for data and metadata — the
package does not duplicate metadata locally.
...
Pipelines are published as whole repositories, not as one script per
dataset, because a study normally produces several datasets across many
...
`replication_scripts/` is a stale local copy of code that now lives in
the org repos. It is in `.Rbuildignore` and should not be extended.
...
- Use `cli` for all user-facing messages (`cli_inform`, `cli_warn`,
  `cli_abort`)
- Progress for downloads via `cli_progress_step()`
- No [`message()`](https://rdrr.io/r/base/message.html),
  [`warning()`](https://rdrr.io/r/base/warning.html), or
  [`stop()`](https://rdrr.io/r/base/stop.html) calls in user-facing code
...
- No internet calls in tests — use
  ...
  or mock
- `R CMD check` passes with 0 errors, 0 warnings, 0 notes (except
  maintainer note)
...
[`get_dataset()`](https://inspercidades.github.io/inspercidados/reference/get_dataset.md)
skips any format whose package is missing, so the package stays usable
with none of the Suggests installed.
